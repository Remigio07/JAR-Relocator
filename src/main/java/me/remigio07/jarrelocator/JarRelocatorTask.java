...
/**
 * A task that copies {@link JarEntry jar entries} from a {@link JarFile jar
 * input} to a {@link JarOutputStream jar output}, applying the relocations
 * defined by a {@link RelocatingRemapper}.
 */
final class JarRelocatorTask {
	 ...
	 *
	 * <a href=
	 * "https://docs.oracle.com/en/java/javase/11/docs/specs/jar/jar.html#signed-jar-file">Specification</a>
	 */
	private static final Pattern SIGNATURE_FILE_PATTERN = Pattern
			.compile("META-INF/(?:[^/]+\\.(?:DSA|RSA|SF)|SIG-[^/]+)");
	...
	void processEntries() throws IOException {
		for (Enumeration<JarEntry> entries = this.jarIn.entries(); entries.hasMoreElements();) {
			...
			// packages
			// defined in a jar. Instead of relocating the entries in it, we delete it,
			// since it is
			// optional anyway.
			//
